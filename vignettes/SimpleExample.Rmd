...
interpret fashion. We can also use the same function to evaluate performance 
on the test data using a named list of new data:
...
As we can see, the model performs less effectively on the test data. 

# Model Accuracy

```{r modacc}

test2 <- modAcc(fullModel, datatype = "test", testdata = list(preds = test[, 1:22], 
                                                     class = test[, 23]))

summary(dfExtract(test2))
```