...
confuse_mat <- function(x,thresh,prop, testdata,...){
  if(!missing(testdata)){
    dv <- as.character(x$formula[[2]])
    mod.mat <- model.matrix(x, data=testdata)
    testdata <- testdata[match(row.names(mod.mat), row.names(testdata)),]
    rm(mod.mat)
    yhat <- predict(x, newdata=testdata, type="response")
    testdata$y <- testdata[, match(dv, colnames(x$data))]
    testdata$yhat <- yhat
    ...
    if(is.null(x$y)){
      dv <- as.character(x$formula[[2]])
      x$model <- model.matrix(x, data=x$data)
      x$model <- na.omit(x$model)
      x$y <- x$data[match(row.names(x$model),row.names(x$data)), 
                    match(dv, colnames(x$data))]
    }
    tp <- length(x$y[x$y > 0 &  x$fitted.values >= thresh])
    fp <- length(x$y[x$y < 1 &  x$fitted.values >= thresh])
    fn <- length(x$y[x$y > 0 &  x$fitted.values <= thresh])
    tn <- length(x$y[x$y < 1 &  x$fitted.values <= thresh])
    mat <- data.frame("Predicted NC" = c(tn, fn), "Predicted C" = c(fp, tp))
    row.names(mat) <- c("Actual NC", "Actual C")
